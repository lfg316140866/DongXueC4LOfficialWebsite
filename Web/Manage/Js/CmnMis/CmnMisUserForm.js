...
/// <reference path="CmnMisRowControl.js" />
...
CmnMis.UserForm =  function () {   
    ...
    this.GetRecList = function ( successFunc, errorFunc, curPageNo) {
        ...
        var _ajaxParam = _Self.GetRecAjaxParam();

        //csv的下载链接
        if (_Self.AllowToExcel == "1") {
            var _downUrl = CmnMis.Func.GetItfUrl(_Self, _Self.Cfg.MethodName.GetRecList);
            ...
            if (CmnMis.Func.GetSysCfg("OutputFileFormat").toLowerCase() == "excel") {
                $(_Self.GetSelector(_Self.Selector.btnDownloadCsv) + " span").eq(1).html("导出Excel");
                _downUrl = Cmn.Func.AddParamToUrl(CmnAjax.Func.GetProxyUrl(_downUrl), "method=GetExcel&FileName=" + _Self.UserFormID);
            }
            else { _downUrl = Cmn.Func.AddParamToUrl(CmnAjax.Func.GetProxyUrl(_downUrl), "method=GetCSV"); }

            //设置下载csv链接
            $(_Self.GetSelector(_Self.Selector.btnDownloadCsv)).attr("href", _downUrl);
        }
        ...
        _Self.DataPaging = new CmnMis.TableOpt.DataPaging(_Self.GetSelector(_Self.Selector.RecContainer),
            ...
            function (data) {
                ...
                if (_IsHasSortCol) { //存在排序列
                    ...
                    var SortUpDownClick = function (sender,isUp) {
                        ...
                        if (isUp===false && $(_tmpDom).next().length > 0) {
                            ...
                            var _param = _Self.GetRecAjaxParam();
                ...
                for (var _i = 0; _i < _Self.ColInfoRecList.length; _i++) {
                    ...
                    //判断是否是数据行控件
                    if (_colInfo["ControlCfg"]["ControlType"] == "RowControl") {
                        if (_colInfo["RowControl"] == undefined) {
                            var _controlDefine = CmnMis.UI.Control[_colInfo["ColControlName"]];

                            _colInfo["RowControl"] = new _controlDefine(_colInfo["ColName"], _colInfo["ControlCfg"], _Self.KeyColName);
                        }

                        _colInfo["RowControl"].Init(data);
                       
                        _Self.AfterRecListLoad.Add(function () { _colInfo["RowControl"].AfterFillData(); }, _colInfo["ColName"]);
    ...
    this.GetControlByName = function ( name) {
        ...
        var _control = $(_Self.GetSelector(_Self.Selector.EditControlPanel))
            .find(CmnMis.UI.Control.Selector.Container + "[name='" + name + "']");
        
        //兼容之前的版本
        if (_control.length <= 0) {
            _control =  $(_Self.GetSelector(_Self.Selector.EditControlPanel))
