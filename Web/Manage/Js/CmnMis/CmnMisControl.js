        ...
        NewControl: function (controlContainer, controlType, colName, controlCfg) {
            ...
            if (!_controlTypeSignature) {
                ...
                if (!_controlHtml && _controlType) {//需要加载
                    ...
                    //获取空间html模板
                    var _controlHtml = CmnAjax.GetFile(CmnMis.UI.Control.CustomControlPath + _controlType + "/" + _controlType + ".htm");
                        

                    //模板存在并且是字符串的时候
                    if (Cmn.IsType(_controlHtml, "string") && _controlHtml != "") {
                        //模板对象
                        var _ctlTemp = $("<div style='display:none;'>" + _controlHtml + "</div>"),
                            _styleLinks = _ctlTemp.find("link").remove(),
                            _styles = _ctlTemp.find("style").remove();

                        //默认先放到body里面去
                        $("body").append(_ctlTemp);

                        //遍历style然后append到最后一个style标签后面
                        _styles.each(function () { $(this).insertAfter($("head style").last()); });

                        //遍历引用的样式文件 并且拼接好link的href 然后append到最后一个link标签后面
                        _styleLinks.each(function () {
                            var _url = $(this).attr("href");
                            $(this).insertAfter($("head link").last());
                            $(this)[0].href = CmnMis.UI.Control.CustomControlPath + _controlType + "/" + _url;
                        });
                        
                        _controlHtml = _ctlTemp.html();
                        _ctlTemp.remove();
                        //暂时先别删除 上面的代码有待观察性能 和调试 下面的代码留在这里备用
                        //定义循环变量和临时变量url以及样式表集合 以及元素块的集合
                        //var _i, _url, _link,_styleLinks = [],_styles=[];

                        ////找到里面所有的 <link href="../xxx/xxx/xxx.css" rel="stylesheet" />
                        //_styleLinks = _controlHtml.match(new RegExp("<link[^>]+\/>", "g"));
                        ////找到里面所有的<style> .xxx{xxx:xxx}</style>
                        //_styles = _controlHtml.match(new RegExp("<style[\s\S]+>[\s\S]+\/style>", "g"));
                        ////缓存控件html模板
                        //_controlHtml = _controlHtml.replace(new RegExp("<style[\s\S]+>[\s\S]+\/style>", "g"), "");
                        //_controlHtml = _controlHtml.replace(new RegExp("<link[^>]+\/>", "g"), "");


                        ////遍历引用的样式文件 并且拼接好link的href 然后append到最后一个link标签后面
                        //if (Cmn.IsType(_styleLinks, "array")) {
                        //    for (_i = 0; _i < _styleLinks.length; _i++) {
                        //        _link = $(_styleLinks[_i]);
                        //        _url = _link.attr("href");
                        //        _link.attr("href", CmnMis.UI.Control.CustomControlPath + _controlType + "/" + _url);
                        //        _link.insertAfter($("link").last());
                        //    }
                        //}
                        ////遍历style然后append到最后一个style标签后面
                        //if (Cmn.IsType(_styles, "array")) {
                        //    for (_i = 0; _i < _styles.length; _i++) {
                        //        $(_styles[_i]).insertAfter($("style").last());
                        //    }
                        //}

                    }
                    else {
                        Cmn.DebugLog("创建控件的时候控件的html模板文件没找到！");
                        _controlHtml = "";
                    }

                    //设置控件的html
                    CmnMis.UI.Control.CustomControlCache[_controlType] = _controlHtml;
                    //加载js
