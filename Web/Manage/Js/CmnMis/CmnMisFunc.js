...
CmnMis.Func = {
    ...
    GetDetailUserFormInfoByID:function(userFormID) {
        ...
        //这个地方需要设置下，因为同一个明细表单可能被多个用户管理界面引用, add at 2015/6/26
        _retUserFormInfo.ParentUserFormID = CmnMis.CurUserForm.UserFormID; //设置父用户界面代码
    ...
    SysCfgCache:{},
    //获取系统配置
    GetSysCfg: function (cfgKey,isUseCache) {
        /// <summary>获取系统配置</summary>
        /// <param name="cfgKey" type="String">键</param>
        /// <param name="isUseCache" type="bool">是否使用缓存(默认为启用缓存)</param>
        /// <returns type="String" />

        if (cfgKey == "" || cfgKey == undefined || cfgKey == null) { return ""; }

        if (isUseCache == undefined) { isUseCache = true; }

        var _retVal = null;

        if (isUseCache) { _retVal = CmnMis.Func.SysCfgCache[cfgKey]; }

        if (_retVal == null || _retVal == undefined) {
            var _cfgValue = CmnAjax.GetData(Cmn.Func.AddParamToUrl(CmnMis.Func.GetCmnItfUrl(), "method=GetSysCfg"), "{CfgKey:'" + cfgKey + "'}");

            if (_cfgValue != undefined && _cfgValue.IsSuccess == "1") {
                _retVal = _cfgValue.CfgValue;

                if (_retVal == undefined || _retVal == null) { _retVal = ""; }

                if (isUseCache) { CmnMis.Func.SysCfgCache[cfgKey] = _retVal; }
            }
            else { _retVal = ""; }
        }
